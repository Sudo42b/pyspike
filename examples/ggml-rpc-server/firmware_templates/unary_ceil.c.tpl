//==================================================================
// {{OP_NAME}} (generated) — element-wise ceil = -floor(-x); per-row tile
// Source: test/CEIL/n1s16/n1s16_ceil.c
// Template name: unary_ceil.c.tpl
//==================================================================
...
#define LOAD_READY_TOKEN_BASE 0xCE00u
...
int main(void) {
    ...
    for (uint32_t tile_row = 0; tile_row < ROWS_PER_NEST; tile_row += TILE_MAX_ROWS) {
        ...
        uint32_t tile_ddr_off = nest_off + tile_row * ROW_BYTES;
                ...
                for (uint8_t tid = 0; tid < SPU_NUM_PER_NEST; tid++) {
                    ...
                    __start_thread(tid);
                        if (rows_this_tid > 0u) {
                            ...
                            uint32_t row_off = row_start * ROW_BYTES;
                            uint32_t chunk_elems = rows_this_tid * WIDTH;
                            ...
                            __set_spm_addr(BANK_A, BANK_C, BANK_B, BANK_R);
                            __floor_v(chunk_elems);
