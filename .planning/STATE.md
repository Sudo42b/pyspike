...
Last activity: 2026-05-18 — Completed quick task 260518-ffr: torch DEVICE='cpu' 강제로 5x ABS perf 회귀 차단 (commit 2b0c66e). 진단 pivot 과정: (1) 처음 cuda 통일 시도 Rule 4 stop (atexit ordering WSL2 bug), (2) grep으로 transfer-overhead 가설 falsified, (3) 진짜 원인은 PyTorch CUDA dispatch overhead, (4) cuda-bindings 12.9.6 부산물이 `cuda.is_available()=True` 트리거. **결과: ABS 458s → 94.82s (4.83x), P8 Plan 04 baseline 95s IDEAL 달성, GELU 65.47s PASS, byte-exact 유지.** 직전 활동 (2026-05-17 quick 260517-s9k): sloop/tloop credit dequeue + sloop_buffer.py 신규 + `_verify.py` 156줄 복원. 그 직전 (2026-05-17 /gsd:debug): D1-D5 5-defect cascade fix + spr_router.py 117줄 삭제 (gtx extension silent ImportError 회귀 해결, 3 commits 2f4c514/e782b35/c24553d). 다음 예정 작업: 12개 `#!TODO` 마커 처리 (dma.py 4 mcast/copy.mem stubs + 5 perf hints + spr.py 3 verification flags).
...
| 260515-mie | Plan invariant assert (WarpState sentinels + control.py 6 asserts) — **reverted**. 사용자 모델 "한 plan 안 thread 1번"이 ABS firmware에 over-strict로 판명(NEST=plan/SPU=thread section). 옵션 B 완화 적용했으나 ABS Gate 2에서 byte-mismatch. revert 후 재측정 = 동일 mismatch → plan invariant와 ABS 무관, ABS는 cleanup arc 진행 중 이미 broken. plan invariant는 abandoned 아니라 보류. | 2026-05-15 | 725b2aa,15a9d19 | [260515-mie-plan-invariant-assert-warpstate-sloop-tl](./quick/260515-mie-plan-invariant-assert-warpstate-sloop-tl/) |
| 260517-s9k | sloop/tloop 유닛별 큐 + credit 기반 dequeue ordering (SMU/TMU producer-consumer barrier). 신규 `sloop_buffer.py` + `_sloop_buf` slot (npu/execute/control wiring) + `_credit_ld_chk`/`_credit_st_chk` 문서화된 NOP → credit-gated dequeue 업그레이드 (clamp-at-0 option a). ABS strict PASS 526s (96 tiles 196609 lines byte-exact, 100% invariant 유지). Rule 3 auto-fix: `_verify.py` 156줄 git history(639ddb4^)에서 복원. **Pre-existing baseline 회귀로 분리**: (1) GELU strict — `act.py:298 firmware_act` assert (직전 commit 0de7607 동일 위치), (2) test_deferred_store.py — numpy→cuda fixture mismatch (메모리 'test_deferred_store.py 재작성' followup). | 2026-05-17 | d26e1c5,4aacd76,ba36b5b | [260517-s9k-sloop-tloop-credit-dequeue-ordering-smu-](./quick/260517-s9k-sloop-tloop-credit-dequeue-ordering-smu-/) |
| 260518-ffr | torch device 통일 — 5x ABS perf 회귀 차단. 진단 pivot: 처음 cuda 통일(Option C) 시도 → Rule 4 stop (atexit ordering: WSL2/cuda 환경에서 torch Py_AtExit이 Python atexit보다 먼저 → `ddr_save_to_hex` `cudaErrorInvalidResourceHandle`로 fail → pytest SKIP). grep sweep으로 "hidden CPU pinning" 가설 falsified — hot path는 모두 GPU 내부. 진짜 원인은 PyTorch CUDA dispatch overhead. 사용자 결정 Option A pivot: `config_params.py:10-11 DEVICE = torch.device("cpu")` 강제 + 13줄 rationale. **결과: ABS 458s → 94.82s (4.83x, P8 Plan 04 baseline 95s IDEAL 달성), GELU 65.47s PASS, byte-exact 유지.** cuda-bindings 12.9.6 부산물 자동 무효화 (별도 uv.lock 정리는 deferred). | 2026-05-18 | 2b0c66e | [260518-ffr-torch-device-ddr-cuda-5x-abs-perf](./quick/260518-ffr-torch-device-ddr-cuda-5x-abs-perf/) |
