...
Last activity: 2026-05-16 — ABS .elf regression **RESOLVED** via /gsd:debug. Root cause: `d6f73f9 Architecture Refactoring` (2026-05-13)이 dma.py의 5개 @handler block(load/store/copy/tpose/fill)을 모두 주석 처리, 이후 복원 누락. 결과: firmware의 main DMA opcode가 dispatch entry 0 → 모든 DMA가 silent NOP → DDR dump 전체 zero. 가설(`b464bb4 SPR`)은 빗나감. Fix 2 commits: (1) `bbb8dbb` dma.py 5개 handler block uncomment + GTX_ISS_F7_TPOSE/FILL import 복원 + 심볼명 정정(DMA_ 접두어 제거), (2) `b0d4592` vec.py L0 SASMD/II 따옴표 버그 fix (b464bb4 search-replace 부산물). 검증: smoke 26/26 + ABS byte-exact PASS 9m10s. 잔여 후속(우선순위 순): plan invariant 재land (옵션 B 완화), 다른 op (.elf) 회귀 sweep, test_deferred_store.py 재작성, D-4 stubs 진짜 구현, R3 n1s16 67-case sweep. 메모리 업데이트: `project_abs_pre_existing_regression`(RESOLVED 표시).
