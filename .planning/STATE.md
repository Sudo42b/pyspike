...
Last activity: 2026-05-15 — Quick task 260515-mie (plan invariant assert) **reverted, ABS pre-existing regression 발견**. Plan invariant 변경(WarpState sentinels + control.py 6 asserts) main에 land 후 `45d72f1`/`ed92898`, ABS Gate 2 회귀에서 byte-mismatch `line 1 fp16[0]: ref=134.875 dump=0.0 ulp=22583`. invariant 변경 모두 revert 후 재실행해도 정확히 같은 mismatch — **plan invariant 작업과 ABS는 완전 무관**, ABS는 cleanup arc 진행 중 어딘가에서 이미 broken 상태(가장 유력 candidate `b464bb4 single-source SPR addresses`)였고 .elf 회귀가 opt-in이라 측정 누락. Revert 그대로 유지(`725b2aa`, `15a9d19`), plan invariant는 abandoned가 아니라 보류 — ABS root cause fix 후 옵션 B 완화(sloop strict + tloop counter ≤ GTX_SPU_NUM)로 재land 가능. 측정 부산물: 사용자 모델 정정 — "한 plan 안 thread 1번"은 over-strict, NEST=plan/SPU=thread section 패턴이 실제 vendor 구조. 메모리 `project_plan_invariant_discovery` + `project_abs_pre_existing_regression` 기록. 잔여 후속: ABS regression debug (최우선), plan invariant 재land, test_deferred_store.py 재작성, D-4 stubs 진짜 구현, R3 n1s16 sweep.
