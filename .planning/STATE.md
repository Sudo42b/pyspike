...
Last activity: 2026-05-17 — Completed quick task 260517-s9k: sloop/tloop 유닛별 큐 + credit 기반 dequeue ordering. 신규 `sloop_buffer.py` (sequential replay, no fusion) + `_sloop_buf` slot wiring + `_credit_*_chk` NOP → credit-gated dequeue (option a clamp-at-0). 3 commits (d26e1c5, 4aacd76, ba36b5b). ABS strict PASS 526s (96 tiles byte-exact 유지). flush_deferred_ddr_stores callsite invariant 보존 (4 lines, dma.py/sloop_buffer.py에 0건). Rule 3 auto-fix로 `_verify.py` 156줄 복원 (639ddb4 인트로 deletion 발견). Pre-existing 회귀 분리 추적: GELU strict (act.py:298 assert, 직전 commit 동일) + test_deferred_store.py (메모리 followup 그대로).
