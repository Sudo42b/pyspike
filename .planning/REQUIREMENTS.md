...
- [x] **DMA-03**: S-loop L2→DDR 스토어 deferred-store 큐, `endp`에서 일괄 flush —
  순서가 C++ 동작과 동일
- [x] **DMA-04**: DDR hex I/O 두 모드(`ddr_init_from_file`, `ddr_dump_to_file`)
  — 표준 LTR(기본) + `GTX_DDR_REVERSED=1` (256-bit 버스 워드 역순) 모두 동작
- [x] **DMA-05**: DMA 라운드트립 테스트 — Python에서 L1에 패턴 쓰고 → DDR로 store
...
- [x] **MM-04**: `mxe_accum` per-(NEST,SPU) FP32 영구 상태 — `is_accumulate` 플래그
  존중, `mm.s → mmc.s → mmc` 체인이 C++과 동일하게 누적
- [x] **MM-05**: 첫 .elf 펌웨어 회귀(GEMM 단순) 통과 — DDR dump가
...
- [x] **ACT-02**: 역방향 활성화 (PRELU/GELU/TANH/SIGM): ADDRR → ADDRA — 비대칭
  방향 테이블 명시적으로 구현
- [x] **ACT-03**: Pooling (`exec_pooling`) 전체
- [x] **ACT-04**: Format conversion (`exec_format_cvt`) — FP16/FP32/FP8 + scale/offset
- [x] **ACT-05**: `_imm` 변형 활성화 (L0 경로, funct7=0x28/0x2A/0x2C/0x2D & 4)

### Verification (Op-level + .elf regression)

- [x] **VRF-01**: `verify.py`(388 LOC, FP16 ULP/atol diff) → `riscv.gtx._verify`로
  포팅 + 모듈로 import 가능
