...
**Success Criteria** (what must be TRUE):
  ...
  2. `len(GtxNpu().get_disasms())` returns the same number of entries as `gtx_npu_disasm.inc` (≈140), and a sample of 5 known instructions (mm, mm_s, mm_t, dma_load, wsplit) decode to expected mnemonics in the spike trace.
