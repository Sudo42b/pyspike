"""unit/memory.py contract: torch-free, xp-backed.

Asserts the memory layer contract:
...
def test_memory_py_has_no_torch_references():
    """memory.py source has zero `torch` code tokens (tokenize-based grep)."""
    import io
    import tokenize
    ...
    assert not code_torch_lines, (
        f"memory.py must have zero `torch` code tokens; found at lines "
...
def test_l1_byte_and_l1_f16_alias_same_storage():
    """Writing 0x3C00 to l1_byte produces bytes `[0x00, 0x3C]` (LE);
    reading the same offset through l1_f16 yields `float16(1.0)`.
    Writes through one view land in the other (shared xp storage).
    """
    mem = GtxMemory()
    ...
    assert _is_xp_ndarray(l1_byte), f"got {type(l1_byte)}"
    assert _is_xp_ndarray(l1_f16), f"got {type(l1_f16)}"
    assert l1_byte.dtype == xp.uint8, f"got {l1_byte.dtype}"
    assert l1_f16.dtype == xp.float16, f"got {l1_f16.dtype}"
    ...
    # Reverse direction: write through underlying storage, read through both views.
    mem.reset_scratchpads()
    l1_byte = mem.l1_byte(nest, spu)
    l1_f16 = mem.l1_f16(nest, spu)
