"""DMA-pattern unit tests against memory.py — backend-agnostic.

Validates that the `DDR_MEMORY.read/write/view` API + the scratchpad
byte/f16 views support the canonical DMA traffic patterns (DDR ↔ L1
byte transfer + FP16 reinterpret) on the xp backend.

These do NOT exercise the dma_engine.py orchestration layer; they only
confirm that the memory layer's contract is correct.
"""
