...
// Project     : GSF / VTS
// Test        : n1s16_geglu_erf
// Description : GEGLU_ERF (GELU-ERF Gated Linear Unit) with
...
//
//               dst[row][i] = gelu_erf(gate[i]) * value[i]
//
//               GELU_ERF(x) = x * 0.5 * (1 + erf(x / sqrt(2)))
//               This is the exact (erf-based) GELU, as opposed to the
//               tanh-approximate variant. The hardware __gelu intrinsic
//               implements the erf-based version, so this kernel is
//               functionally identical to GEGLU.
//
//               Work partitioning:
...
//               Thread: load 1 row (32B) to Bank A, compute GEGLU_ERF
//
//               Compute per SPU:
//                 1. Save gate (A[0..15]) -> C[0..15]
//                 2. Save value (A[16..31]) -> C[16..31]
//                 3. Copy gate A -> R
//                 4. __gelu(WIDTH_OUT) -> gelu_erf(gate) in A  (R -> A)
//                 5. Copy gelu_erf(gate) A -> B
//                 6. Restore value: C+16 -> A
//                 7. final = value * gelu_erf(gate): A * B -> R
...
#ifndef N1S16_GEGLU_ERF_C
#define N1S16_GEGLU_ERF_C
...
int main(void) {
...

    {
        ...
        __start_plan(nest_id);
            ...
            //=====================================================
            // Threads: L2 <-> L1, compute GEGLU_ERF
            //=====================================================
            for (uint8_t tid = 0; tid < SPU_NUM_PER_NEST; tid++) {
                ...
                __start_thread(tid);
                    ...
                    for (uint8_t r = 0; r < ROWS_PER_SPU; r++) {
                        ...
                        //=================================================
                        // GEGLU_ERF computation:
                        // dst[i] = gelu_erf(gate[i]) * value[i]
                        //
                        // gelu_erf(x) = x * 0.5 * (1 + erf(x / sqrt(2)))
                        // Hardware __gelu is erf-based, so we use it directly.
                        //=================================================
                        ...
                        // Step 4: gelu_erf(gate)
                        // __gelu reads from Bank R -> writes Bank A
                        // Hardware __gelu implements erf-based GELU
                        __gelu(WIDTH_OUT);
                        // Now: A = gelu_erf(gate)

                        // Step 5: Copy gelu_erf(gate) A -> B
                        ...
                        // Step 7: final = value * gelu_erf(gate): A * B -> R
                        __mul_vv(WIDTH_OUT);
