//==================================================================
// n1s16_acc — GTX n1s16 ACC kernel for the current contiguous full-span
// harness subset, i.e. ggml_acc with offset 0 over the whole destination,
// which reduces to dst[row] = src0[row] + src1[row] for HEIGHT x WIDTH FP16.
//==================================================================
...
static inline uint32_t rows_for_tid(uint32_t total_rows, uint8_t tid) {
    uint32_t quot = total_rows / SPU_NUM_PER_NEST;
    uint32_t rem = total_rows % SPU_NUM_PER_NEST;
    return (uint32_t) (tid < rem ? (quot + 1) : quot);
}
...
int main(void) {
    ...
    uint32_t nest_off = TARGET_NEST_ID * NEST_DATA_BYTES;
    ...
    for (uint32_t tile_row_start = 0; tile_row_start < ROWS_PER_NEST; tile_row_start += SHARED_TILE_MAX_ROWS) {
        ...
        {
            __start_plan(TARGET_NEST_ID);

                __start_shared();
                    __load(
                        GTX_MAIN_ADDR(BASE_DDR_A) + tile_ddr_off, L2_A,
                        tile_bytes, (uint16_t) tile_bytes, 1, (uint16_t) tile_bytes
                    );

                    __load_cr(
                        GTX_MAIN_ADDR(BASE_DDR_B) + tile_ddr_off, L2_B,
                        tile_bytes, (uint16_t) tile_bytes, 1, (uint16_t) tile_bytes,
                        1, active_tid_mask, SHARED_READY_TOKEN
                    );

                    __credit_chk(active_tid_mask);

                    __store_cr(
                        L2_RESULT, GTX_MAIN_ADDR(BASE_DDR_RESULT) + tile_ddr_off,
                        tile_bytes, (uint16_t) tile_bytes, 1, (uint16_t) tile_bytes,
                        1, active_tid_mask
                    );
                __end_shared();
                ...
                for (uint8_t tid = 0; tid < SPU_NUM_PER_NEST; tid++) {
                    ...
                    __start_thread(tid);
                        ...
                        if (rows_for_this_tid > 0) {
                            __credit_chk(SHARED_READY_TOKEN);
                                ...
                                __load(
                                 ...
                                 if (r == rows_for_this_tid - 1) {
                                     __load_cr(
                                         L2_B + row_off, BANK_B,
                                         ROW_BYTES, (uint16_t) ROW_BYTES, 1, (uint16_t) ROW_BYTES,
                                         1, (uint16_t) (0x1u << tid), TARGET_NEST_ID
                                     );
                                 } else {
                                    __load(
                                        L2_B + row_off, BANK_B,
