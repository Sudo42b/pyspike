//==================================================================
// n1s16_elu — element-wise ELU with 1 NEST x 16 SPUs
// dst[row] = (x > 0) ? x : exp(x) - 1, 64 rows x 8 FP16 elements
// Implemented as a composed GTX path:
//   max(x, 0) + min(exp(x) - 1, 0)
//==================================================================
            ...
            for (uint8_t tid = 0; tid < SPU_NUM_PER_NEST; tid++) {
                ...
                __start_thread(tid);
                    ...
                    for (uint8_t r = 0; r < ROWS_PER_SPU; r++) {
                        ...
                        // A has x; save x to C for the negative branch.
                        __copy(BANK_A, BANK_C, ROW_BYTES, ROW_BYTES, 1, ROW_BYTES);
                        // R = max(x, 0); save positive branch in B.
                        __clamp_min(WIDTH, FP16_ZERO, 0);
                        __copy(BANK_R, BANK_B, ROW_BYTES, ROW_BYTES, 1, ROW_BYTES);

                        // Negative branch from original x.
                        __copy(BANK_C, BANK_A, ROW_BYTES, ROW_BYTES, 1, ROW_BYTES);
                        __exp_v(WIDTH, 0);                                  // R = exp(x)
                        __copy(BANK_R, BANK_A, ROW_BYTES, ROW_BYTES, 1, ROW_BYTES);
                        __sub_vs(WIDTH, FP16_ONE, 0);                        // R = exp(x) - 1
                        __copy(BANK_R, BANK_A, ROW_BYTES, ROW_BYTES, 1, ROW_BYTES);
                        __clamp_max(WIDTH, FP16_ZERO, 0);                    // R = min(exp(x)-1, 0)
                        __copy(BANK_R, BANK_A, ROW_BYTES, ROW_BYTES, 1, ROW_BYTES);
                        __add_vv(WIDTH);                                     // R = ELU(x)
