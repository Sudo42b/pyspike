//==================================================================
// n1s16_add1 — add scalar to all elements, 1 NEST x 16 SPUs
// dst[row] = src0[row] + scalar, HEIGHT rows x WIDTH FP16 elements
//
// Scalar (1 FP16) is read from DDR at BASE_DDR_B before __split.
// Shared mode tiles rows through L2, then SPUs process assigned rows in L1
// with the native vector-scalar add intrinsic.
//==================================================================
...
#define L2_RESULT             SHARED_TILE_MAX_BYTES
...
int main(void) {
    ...
    volatile uint16_t *scalar_ptr = (volatile uint16_t *)GTX_MAIN_ADDR(BASE_DDR_B);
    uint16_t scalar_val = *scalar_ptr;
    uint8_t nest_id = 0;
                    ...
                    __start_thread(tid);
                        ...
                        if (rows_for_this_tid > 0) {
                            ...
                            for (uint32_t r = 0; r < rows_for_this_tid; r++) {
                                ...
                                uint32_t is_last_row = (r + 1) == rows_for_this_tid;
