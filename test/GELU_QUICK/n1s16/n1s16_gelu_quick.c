//==================================================================
// n1s16_gelu_quick — element-wise GELU Quick with 1 NEST x 16 SPUs
// dst[row] = x * sigmoid(1.702*x), 64 rows x 8 FP16 elements
// A->C(save x), mul_vs(A*1.702->R), sigm(R->A), A->B, C->A, mul_vv(A*B->R)
            ...
            for (uint8_t tid = 0; tid < SPU_NUM_PER_NEST; tid++) {
                __start_thread(tid);
                    ...
                    for (uint8_t r = 0; r < ROWS_PER_SPU; r++) {
                        ...
                        // 1.702*x: A * scalar -> R
                        __mul_vs(WIDTH, 0x3ED1, 0);

                        // sigmoid(1.702*x): reads R, writes A = sigmoid(1.702*x)
                        __sigm(WIDTH);
