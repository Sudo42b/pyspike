//==================================================================
// n1s16_floor — element-wise floor with 1 NEST x 16 SPUs
// dst[row] = floor(src0[row]), HEIGHT rows x WIDTH FP16 elements
//==================================================================
...
#define EXTRA_ROWS          (ROWS_PER_NEST % SPU_NUM_PER_NEST)
...
int main(void) {
    ...
    for (uint8_t tid = 0; tid < SPU_NUM_PER_NEST; tid++) {
        uint32_t rows_for_tid = BASE_ROWS_PER_SPU + ((tid < EXTRA_ROWS) ? 1u : 0u);
        if (rows_for_tid != 0) {
            active_mask |= (uint16_t)(0x1u << tid);
        }
    }
...

    {
        ...
        __start_plan(nest_id);
            ...
            for (uint8_t tid = 0; tid < SPU_NUM_PER_NEST; tid++) {
                uint32_t rows_for_tid = BASE_ROWS_PER_SPU + ((tid < EXTRA_ROWS) ? 1u : 0u);
                uint32_t start_row = (uint32_t)tid * BASE_ROWS_PER_SPU + ((tid < EXTRA_ROWS) ? tid : EXTRA_ROWS);
                ...
                __start_thread(tid);
                    ...
                    for (uint32_t r = 0; r < rows_for_tid; r++) {
                        uint32_t row_off = (start_row + r) * ROW_BYTES;
