...
// Project     : GSF / VTS
// Test        : n1s16_diag_mask_zero (1 NEST x 16 SPUs = 16 SPUs)
// Description : Causal mask with zero (upper triangular masking)
//
//               src:  [64][8] FP16 at 0x1000000 (1024B)
//               dst:  [64][8] FP16 at 0xf000000 (1024B)
//
//               For row i, column j:
//                 if j > i:  dst[i][j] = 0 (FP16 0x0000)
...
//
//               Strategy:
//               - Each SPU loads its row to Bank A, copies to Bank R.
//               - For rows that need masking (global_row < 7):
//                 fill the tail portion of Bank R with zeros via __fill.
//
// Author      : sw.lee
//==================================================================

#ifndef N1S16_DIAG_MASK_ZERO_C
#define N1S16_DIAG_MASK_ZERO_C
...
// L2 SPM addresses
#define L2_A                0x000000    // 16 rows x 16B = 256B per NEST
#define L2_RESULT           0x002000    // 16 rows x 16B = 256B per NEST
...
int main(void) {
...

    {
        ...
        __start_plan(nest_id);
            ...
            //=========================================================
            // Threads: apply causal mask with zero
            //=========================================================
            for (uint8_t tid = 0; tid < SPU_NUM_PER_NEST; tid++) {
                ...
                __start_thread(tid);
                    ...
                    for (uint8_t r = 0; r < rows_for_tid; r++) {
                        ...
                        // Apply mask: if global_row < WIDTH, overwrite tail with zero
                        ...
                        if (global_row + 1 < WIDTH) {
                            ...
                            // Fill zero into the tail of Bank R
                            // NOTE: __fill is shared-only; use __wrspr+__store_svr in thread
                            uint64_t zero_word = 0ULL;
                            __wrspr(SVR_ADDR + 0*4,     0, zero_word, 0);
                            __wrspr(SVR_ADDR + 0*4 + 1, 0, zero_word, 0);
                            __wrspr(SVR_ADDR + 0*4 + 2, 0, zero_word, 0);
                            __wrspr(SVR_ADDR + 0*4 + 3, 0, zero_word, 0);
                            // Store 32B of zeros; copy only mask_bytes to the right offset
                            __store_svr(BANK_C, 0);
                            __copy(BANK_C, BANK_R + mask_offset, 0, mask_bytes, 1, 0);
                        }
